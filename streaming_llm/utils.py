...
def parse_args():
    ...
    parser.add_argument(
        "--split", type=str, default="test", choices=["validation", "test"]          #验证集、测试集
    )
...
def load(model_name_or_path):
    ...
    model = AutoModelForCausalLM.from_pretrained(
        model_name_or_path,
        device_map="auto",              #自动将模型不同层分配到可用设备上
        torch_dtype=torch.float16,      #模型权重加载为半精度浮点数
        trust_remote_code=True,
    )
